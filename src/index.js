...
async function scanSite(rawUrl) {
  ...
  // --- DETECT SECURITY/BOT-CHECK WALL (Cloudflare etc.) ---
  // If we only got the "Just a moment..." interstitial, the real page wasn't read.
  // Be honest rather than report false failures.
  const looksBlocked =
    /just a moment/i.test(rawHtml) ||
    /checking your browser/i.test(rawHtml) ||
    /cf-browser-verification|cf_chl|challenge-platform/i.test(rawHtml) ||
    /enable javascript and cookies to continue/i.test(rawHtml) ||
    (page.status && (page.status === 403 || page.status === 503));
  if (looksBlocked) {
    add('Note', 'Limited scan', 'info', 'This site is protected by a security layer (like Cloudflare) that blocks automated scanners. We could confirm it loads and is secure, but could not read the full page content — so a detailed automated scan isn\'t possible here. A specialist can review it manually on a free call.');
    add('Security', 'HTTPS secure connection', isHttps ? 'good' : 'warn',
      isHttps ? 'Your site uses a secure HTTPS connection.' : 'We could not confirm HTTPS on this site.');
    result.overall = isHttps ? 70 : 50;
    result.summary = 'We could only partially scan this site — it\'s protected by a security layer that blocks automated tools. That\'s common and not necessarily a problem. For a full review, book a free call and we\'ll go through it together.';
    result.note = 'This was a limited scan. Some details could not be read automatically — book a free call for a full, personalized review.';
    result.limited = true;
    return result;
  }
  ...
  add('SEO', 'Meta description', descContent ? 'good' : 'warn',
    descContent ? 'Meta description is present (helps your search listing).' : 'We couldn\'t find a meta description in the page code. Adding one helps control the text Google shows under your search result. (If your site loads content dynamically, it may exist but not be readable here.)');

  const h1Count = (html.match(/<h1[\s>]/g) || []).length;
  add('SEO', 'Heading structure (H1)', h1Count === 1 ? 'good' : h1Count === 0 ? 'warn' : 'warn',
    h1Count === 1 ? 'Has a clear main heading (H1).' : h1Count === 0 ? 'We couldn\'t detect a main heading (H1) in the page code — it helps search engines understand your page. (May not be readable if your site loads content dynamically.)' : `Found ${h1Count} H1 headings — ideally there is just one.`);
  ...
  add('Search Visibility', 'Location & contact info on page', (hasAddress && hasPhone) ? 'good' : 'warn',
    (hasAddress && hasPhone) ? 'Your address and phone are detectable in the page text — good for local search.' :
    'We couldn\'t detect your address and phone as text on the page. If they\'re shown inside an image or load after the page opens, search engines may not read them either — worth making them plain text. (If they\'re already there as text, you can disregard this.)');
